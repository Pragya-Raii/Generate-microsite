...
import { Upload, X, Sparkles, Image, Send, AlertCircle, Loader, ArrowLeft, RotateCcw, User, Menu, FileText } from 'lucide-react';
...
const PromptInput = ({
  ...
  }, [prompt, imageAnalysis, pdfAnalysis]);
  ...
  const handlePdfUpload = (event) => {
    ...
    if (file) {
      ...
      if (file.size > maxSize) {
        alert('PDF file is too large. Please upload a file smaller than 20MB.');
        return;
      }
  ...
  const canGenerate = () => {
    if (uploadedImage) {
      ...
      return pdfAnalysis && !isAnalyzingPdf && !isGenerating;
    } else {
      ...
      <div className="main-content-area">
        {responsePhase === 'input' ? (
          <>
            ...
            {/* Prompt Input Area with inline image/pdf preview */}
            <div className="prompt-input-area">
              <div className="input-container">
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: (uploadedImage || uploadedPdf) ? '0.5rem' : 0 }}>
                  {uploadedImage && (
                  ...
                  {uploadedPdf && (
                    <div style={{ position: 'relative', width: 36, height: 36, display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#fef2f2', borderRadius: 6, border: '1px solid #fee2e2' }}>
                ...
                <textarea
                  ref={textareaRef}
                  value={(imageAnalysis && !isAnalyzing) ? imageAnalysis.description : (pdfAnalysis && !isAnalyzingPdf) ? pdfAnalysis.description : prompt}
                  onChange={(e) => {
                    setPrompt(e.target.value);
                    adjustTextareaHeight();
                  }}
                  placeholder="Describe your website idea here..."
                  disabled={isGenerating || isAnalyzing || isAnalyzingPdf}
                ...
                <div className="input-buttons">
                  ...
                  <button
                    onClick={onGenerate}
                    disabled={!canGenerate() || isAnalyzing || isAnalyzingPdf}
                    className="send-icon-button"
                    title={uploadedImage ? 'Generate from Image' : uploadedPdf ? 'Generate from PDF' : 'Generate Website'}
                  >
                    {isGenerating ? (
              ...
              <div className="chat-messages">
                ...
                <div className="message user-message">
                  <div className="message-content" style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    {uploadedPdf && (
                      <div style={{ width: 32, height: 32, display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#fef2f2', borderRadius: 6, border: '1px solid #fee2e2', marginRight: 8 }}>
